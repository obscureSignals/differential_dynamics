...
std::vector<torch::Tensor> ssl_smoother_backward(
  ...
  for (int64_t b = 0; b < B; ++b) {
    ...
    bool salt_on = false;
    if (const char* ss = std::getenv("SSL_USE_SALTATION")) {
      salt_on = (ss[0]=='1'||ss[0]=='t'||ss[0]=='T'||ss[0]=='y'||ss[0]=='Y');
    }
    ...
    std::vector<float> lam1_hist_post, lam2_hist_post;
    if (step_dbg || salt_on) {
    ...
    for (int64_t t = T - 1; t >= 0; --t) {
      ...
      l1 += gy_ptr[t];
      l2 += gy_ptr[t];
      if (step_dbg || salt_on) { lam1_hist[t] = l1; lam2_hist[t] = l2; }
...
// Decide gradient mode: analytic (default) or FD if env requests
    ...
    // Chain rule from rates to time constants: R = 1/T => dL/dT = -dL/dR / T^2
      ...
      if (salt_on) {
        ...
        const double gT_as_pre = (double)gT_as.data_ptr<float>()[b];
        const double gT_sf_pre = (double)gT_sf.data_ptr<float>()[b];
        const double gT_ss_pre = (double)gT_ss.data_ptr<float>()[b];
        // Epsilon/backoff for local delta sensitivities wrt ln T
        double eps_rel_salt = 1e-3;
        ...
        if (const char* ls = std::getenv("SSL_SALTATION_LOG_SUMMARY")) { log_summary = (ls[0]=='1'||ls[0]=='t'||ls[0]=='T'||ls[0]=='y'||ls[0]=='Y'); }
        if (log_summary && b == 0) {
          int flip_count = 0;
          for (int64_t tt = 1; tt < T; ++tt) if (at_mask[tt] != at_mask[tt-1]) flip_count++;
          std::printf("[salt] enabled: B=%lld T=%lld flips=%d eps_rel=%.3g max_backoff=%d max_flips=%d\n",
                      (long long)B, (long long)T, flip_count, eps_rel_salt, max_backoff_salt, max_flips);
          std::fflush(stdout);
        }

        // Helper: compute full delta sequence for given time constants (fixed mask and g_arr)
        auto delta_seq_for_Ts = [&](double Taf_v, double Tas_v, double Tsf_v, double Tss_v) -> std::vector<double> {
          Taf_v = std::max(Taf_v, 1e-12);
          Tas_v = std::max(Tas_v, 1e-12);
          Tsf_v = std::max(Tsf_v, 1e-12);
          Tss_v = std::max(Tss_v, 1e-12);
          const double Raf_v = 1.0 / Taf_v;
          const double Ras_v = 1.0 / Tas_v;
          const double Rsf_v = 1.0 / Tsf_v;
          const double Rss_v = 1.0 / Tss_v;
          ...
          double x1l=0.0, x2l=0.0, yprevl=0.0;
          for (int64_t s = 0; s < T; ++s) {
            const double a_s = (double)g_arr[s];
            const bool is_att = (at_mask[s] != 0);
            const DiscreteAB& ABs = is_att ? ABa : ABr;
            const double nx1 = (double)ABs.Ad11 * x1l + (double)ABs.Ad12 * x2l + (double)ABs.Bd1 * a_s;
            const double nx2 = (double)ABs.Ad21 * x1l + (double)ABs.Ad22 * x2l + (double)ABs.Bd2 * a_s;
            // delta based on state BEFORE updating (uses yprevl)
            delta[s] = a_s - yprevl;
            x1l = nx1; x2l = nx2;
            yprevl = x1l + x2l;
          }
        ...
        // Compute d/d ln T of delta sequence with adaptive epsilon (central difference)
        auto dlnT_delta_seq = [&](int which_T) -> std::vector<double> {
          double eps_rel = eps_rel_salt;
          for (int attempt = 0; attempt <= max_backoff_salt; ++attempt) {
            double e = eps_rel;
            double Taf_p=Taf, Tas_p=Tas, Tsf_p=Tsf, Tss_p=Tss;
            double Taf_m=Taf, Tas_m=Tas, Tsf_m=Tsf, Tss_m=Tss;
            if (which_T==0) { Taf_p=Taf*std::exp(+e); Taf_m=Taf*std::exp(-e); }
            else if (which_T==1) { Tas_p=Tas*std::exp(+e); Tas_m=Tas*std::exp(-e); }
            else if (which_T==2) { Tsf_p=Tsf*std::exp(+e); Tsf_m=Tsf*std::exp(-e); }
            else { Tss_p=Tss*std::exp(+e); Tss_m=Tss*std::exp(-e); }
            auto dp = delta_seq_for_Ts(Taf_p, Tas_p, Tsf_p, Tss_p);
            auto dm = delta_seq_for_Ts(Taf_m, Tas_m, Tsf_m, Tss_m);
            std::vector<double> g(T);
            bool ok = true;
            for (int64_t s = 0; s < T; ++s) {
              g[s] = (dp[s] - dm[s]) / (2.0 * e);
              if (!std::isfinite(g[s])) { ok = false; break; }
            }
            if (ok) return g;
            eps_rel *= 0.1;
          }
          // Fallback zeros if non-finite after backoff (safe; FD fallback still covers)
          return std::vector<double>(T, 0.0);
        };

        // Note: Use per-flip branch difference (AB_after - AB_before) to get correct sign for both
        // attack->release and release->attack transitions.

        // Precompute d ln T sensitivities for delta at all t
        auto g_dlnT_af = dlnT_delta_seq(0);
        auto g_dlnT_as = dlnT_delta_seq(1);
        auto g_dlnT_sf = dlnT_delta_seq(2);
        auto g_dlnT_ss = dlnT_delta_seq(3);

        // Helper: one-step split update with flip at tau within the sample
        auto step_with_split = [&](double tau_s, bool before_attack, bool after_attack,
                                   double x1_t, double x2_t, double a_t) -> std::pair<double,double> {
          if (tau_s < 0.0) tau_s = 0.0;
          if (tau_s > (double)Ts) tau_s = (double)Ts;
          const double dur1 = tau_s;
          const double dur2 = (double)Ts - tau_s;
          // Before segment
          DiscreteAB AB1 = before_attack
              ? zoh_discretize_from_rates((float)Raf, (float)Ras, (float)Rsf, (float)Rss, (float)dur1)
              : zoh_discretize_from_rates(0.f, 0.f, (float)Rsf, (float)Rss, (float)dur1);
          // After segment
          DiscreteAB AB2 = after_attack
              ? zoh_discretize_from_rates((float)Raf, (float)Ras, (float)Rsf, (float)Rss, (float)dur2)
              : zoh_discretize_from_rates(0.f, 0.f, (float)Rsf, (float)Rss, (float)dur2);
          // Compose
          double x1_mid = (double)AB1.Ad11 * x1_t + (double)AB1.Ad12 * x2_t + (double)AB1.Bd1 * a_t;
          double x2_mid = (double)AB1.Ad21 * x1_t + (double)AB1.Ad22 * x2_t + (double)AB1.Bd2 * a_t;
          double x1_nxt = (double)AB2.Ad11 * x1_mid + (double)AB2.Ad12 * x2_mid + (double)AB2.Bd1 * a_t;
          double x2_nxt = (double)AB2.Ad21 * x1_mid + (double)AB2.Ad22 * x2_mid + (double)AB2.Bd2 * a_t;
          return {x1_nxt, x2_nxt};
        };

        // Iterate over flip indices (optionally limited)
        int used_flips = 0;
        double salt_af_sum = 0.0, salt_as_sum = 0.0, salt_sf_sum = 0.0, salt_ss_sum = 0.0;
        // Optional debugging and sign test
        bool dbg_salt = false;
        if (const char* ds = std::getenv("SSL_DEBUG_SALTATION")) { dbg_salt = (ds[0]=='1'||ds[0]=='t'||ds[0]=='T'||ds[0]=='y'||ds[0]=='Y'); }
        // Throttle logging: print every Nth flip, up to a maximum per backward
        int log_every = 8192; // very sparse by default
        if (const char* le = std::getenv("SSL_SALTATION_LOG_EVERY")) { int v = std::atoi(le); if (v >= 1) log_every = v; }
        int log_max = 2;      // cap prints per backward
        if (const char* lm = std::getenv("SSL_SALTATION_LOG_MAX")) { int v = std::atoi(lm); if (v >= 0) log_max = v; }
        int printed_flips = 0;
        bool sign_test_flip = false;
        if (const char* st = std::getenv("SSL_SALTATION_SIGN_TEST")) { sign_test_flip = (st[0]=='1'||st[0]=='t'||st[0]=='T'||st[0]=='y'||st[0]=='Y'); }
        const double sign_scale = sign_test_flip ? -1.0 : 1.0;
        for (int64_t t = 1; t < T; ++t) if (at_mask[t] != at_mask[t-1]) {
          if (used_flips >= max_flips) break;
          const double delta_t   = (double)g_arr[t]   - (double)y_prev_arr[t];
          const double delta_tm1 = (double)g_arr[t-1] - (double)y_prev_arr[t-1];
          const double ddelta_dt = (delta_t - delta_tm1) / (double)Ts; // per-second slope
          if (!std::isfinite(ddelta_dt) || std::fabs(ddelta_dt) < 1e-12) { used_flips++; continue; }
          // Estimate flip location within the sample (alpha in (0,1)) via linear interpolation
          double denom = (delta_t - delta_tm1);
          if (denom == 0.0) { used_flips++; continue; }
          double alpha = - delta_tm1 / denom; // fraction from (t-1)->t where delta crosses 0
          // Clamp alpha to [eps, 1-eps]
          double eps_a = 1e-6;
          if (!(alpha > eps_a && alpha < (1.0 - eps_a))) {
            alpha = std::min(std::max(alpha, eps_a), 1.0 - eps_a);
          }
          const double tau0 = alpha * (double)Ts;

          // Numeric derivative of x_{t+1} wrt tau via split-step composition
          auto nx = [&](double tau)->std::pair<double,double>{
            bool before_attack = (at_mask[t-1] != 0);
            bool after_attack  = (at_mask[t]   != 0);
            return step_with_split(tau, before_attack, after_attack, (double)x1_prev[t], (double)x2_prev[t], (double)g_arr[t]);
          };
          // Adaptive epsilon
          double eps_tau = std::max(1e-7, 1e-3 * (double)Ts);
          // Keep within bounds
          double tau_p = std::min((double)Ts - 1e-9, tau0 + eps_tau);
          double tau_m = std::max(1e-9,             tau0 - eps_tau);
          auto nxt_p = nx(tau_p);
          auto nxt_m = nx(tau_m);
        ...
        // Optional one-line summary of smooth vs saltation totals (batch 0)
        bool sum_on = false;
        if (const char* ss = std::getenv("SSL_SALTATION_LOG_SUMMARY")) { sum_on = (ss[0]=='1'||ss[0]=='t'||ss[0]=='T'||ss[0]=='y'||ss[0]=='Y'); }
        if (sum_on && b == 0) {
          const double gT_af_post = (double)gT_af.data_ptr<float>()[b];
          const double gT_as_post = (double)gT_as.data_ptr<float>()[b];
          const double gT_sf_post = (double)gT_sf.data_ptr<float>()[b];
          const double gT_ss_post = (double)gT_ss.data_ptr<float>()[b];
          std::printf("[salt_sum] smooth=(af %.6g, as %.6g, sf %.6g, ss %.6g) salt=(af %.6g, as %.6g, sf %.6g, ss %.6g) total=(af %.6g, as %.6g, sf %.6g, ss %.6g)\n",
                      gT_af_pre, gT_as_pre, gT_sf_pre, gT_ss_pre,
                      salt_af_sum, salt_as_sum, salt_sf_sum, salt_ss_sum,
                      gT_af_post, gT_as_post, gT_sf_post, gT_ss_post);
        }
      }
      ...
      // Robustness: double-precision eval, relative perturbations in log-space, adaptive epsilon backoff, fallback to analytic.
      ...
      // Read knobs (always-on defaults if unset)
      auto env_truthy = [](const char* v){ return v && (v[0]=='1'||v[0]=='t'||v[0]=='T'||v[0]=='y'||v[0]=='Y'); };
      double eps_rel_default = 1e-3; // for ln(T) and ln(R)
      int max_backoff = 4;
      if (const char* e = std::getenv("SSL_TCONST_FD_EPS_REL")) { eps_rel_default = std::atof(e); if (!(eps_rel_default>0.0)) eps_rel_default = 1e-3; }
      if (const char* m = std::getenv("SSL_TCONST_FD_MAX_BACKOFF")) { int k = std::atoi(m); if (k>=0) max_backoff = k; }

      auto scalar_loss_for_Ts = [&](double Taf_v, double Tas_v, double Tsf_v, double Tss_v) -> double {
        Taf_v = std::max(Taf_v, 1e-12);
        Tas_v = std::max(Tas_v, 1e-12);
        Tsf_v = std::max(Tsf_v, 1e-12);
        Tss_v = std::max(Tss_v, 1e-12);
        ...
        for (int64_t t = 0; t < T; t += subsample) {
          const double xdb = (double)x_ptr[t] + (double)fb * yprevl;
          double a = (double)slope * ((double)thresh - xdb);
          if (a > 0.0) a = 0.0;
          const double delta = a - yprevl;
          const bool is_attack = (delta < 0.0);
          const DiscreteAB& AB = is_attack ? ABa : ABr;
          const double nx1 = (double)AB.Ad11 * x1l + (double)AB.Ad12 * x2l + (double)AB.Bd1 * a;
          const double nx2 = (double)AB.Ad21 * x1l + (double)AB.Ad22 * x2l + (double)AB.Bd2 * a;
          x1l = nx1; x2l = nx2;
      ...
      auto fallback_analytic_timeconst = [&](){
        std::printf("[ssl_fd_warn] non-finite FD time-constant gradients; falling back to analytic contraction for this item\n");
        // Reuse already-accumulated operator Jacobian contractions to compute dL/dR and then chain to dL/dT
        // Derivatives of (Ad,Bd) w.r.t rates for attack branch
        DiscreteAB d_att_q0, d_att_q1, d_att_q2, d_att_q3;
        numeric_dAB_drates(Raf, Ras, Rsf, Rss, Ts, d_att_q0, d_att_q1, d_att_q2, d_att_q3);
        DiscreteAB d_rel_q0, d_rel_q1, d_rel_q2, d_rel_q3;
        numeric_dAB_drates(0.f, 0.f, Rsf, Rss, Ts, d_rel_q0, d_rel_q1, d_rel_q2, d_rel_q3);
        auto dotAB_local = [&](const DiscreteAB& d, float gAd11, float gAd12, float gAd21, float gAd22, float gBd1, float gBd2) -> double {
          double s = 0.0;
          s += (double)gAd11 * (double)d.Ad11 + (double)gAd12 * (double)d.Ad12 + (double)gAd21 * (double)d.Ad21 + (double)gAd22 * (double)d.Ad22;
          s += (double)gBd1  * (double)d.Bd1  + (double)gBd2  * (double)d.Bd2;
          return s;
        };
        double dL_dR_af = dotAB_local(d_att_q0, gAd_att_11, gAd_att_12, gAd_att_21, gAd_att_22, gBd_att_1, gBd_att_2);
        double dL_dR_as = dotAB_local(d_att_q1, gAd_att_11, gAd_att_12, gAd_att_21, gAd_att_22, gBd_att_1, gBd_att_2);
        double dL_dR_sf = dotAB_local(d_att_q2, gAd_att_11, gAd_att_12, gAd_att_21, gAd_att_22, gBd_att_1, gBd_att_2)
                        + dotAB_local(d_rel_q2, gAd_rel_11, gAd_rel_12, gAd_rel_21, gAd_rel_22, gBd_rel_1, gBd_rel_2);
        double dL_dR_ss = dotAB_local(d_att_q3, gAd_att_11, gAd_att_12, gAd_att_21, gAd_att_22, gBd_att_1, gBd_att_2)
                        + dotAB_local(d_rel_q3, gAd_rel_11, gAd_rel_12, gAd_rel_21, gAd_rel_22, gBd_rel_1, gBd_rel_2);
        gT_af.data_ptr<float>()[b] = (float)(- dL_dR_af / (Taf * Taf));
        gT_as.data_ptr<float>()[b] = (float)(- dL_dR_as / (Tas * Tas));
        gT_sf.data_ptr<float>()[b] = (float)(- dL_dR_sf / (Tsf * Tsf));
        gT_ss.data_ptr<float>()[b] = (float)(- dL_dR_ss / (Tss * Tss));
      };

      if (!fixed_mask) {
        auto central_diff_logT = [&](double Taf_c, double Tas_c, double Tsf_c, double Tss_c, int which)->double{
          // Compute dL/d ln(T_which) with adaptive epsilon
          double eps_rel = eps_rel_default;
          for (int attempt = 0; attempt <= max_backoff; ++attempt) {
            double e = eps_rel;
            double Taf_p=Taf_c, Tas_p=Tas_c, Tsf_p=Tsf_c, Tss_p=Tss_c;
            double Taf_m=Taf_c, Tas_m=Tas_c, Tsf_m=Tsf_c, Tss_m=Tss_c;
            if (which==0) { Taf_p = Taf_c * std::exp(+e); Taf_m = Taf_c * std::exp(-e); }
            else if (which==1) { Tas_p = Tas_c * std::exp(+e); Tas_m = Tas_c * std::exp(-e); }
            else if (which==2) { Tsf_p = Tsf_c * std::exp(+e); Tsf_m = Tsf_c * std::exp(-e); }
            else { Tss_p = Tss_c * std::exp(+e); Tss_m = Tss_c * std::exp(-e); }
            double Lp = scalar_loss_for_Ts(Taf_p, Tas_p, Tsf_p, Tss_p);
            double Lm = scalar_loss_for_Ts(Taf_m, Tas_m, Tsf_m, Tss_m);
            double num = Lp - Lm;
            double den = 2.0 * e;
            double g = num / den;
        ...
        // Compute grads
        double g_lnT_af = central_diff_logT((double)Taf, (double)Tas, (double)Tsf, (double)Tss, 0);
        double g_lnT_as = central_diff_logT((double)Taf, (double)Tas, (double)Tsf, (double)Tss, 1);
        double g_lnT_sf = central_diff_logT((double)Taf, (double)Tas, (double)Tsf, (double)Tss, 2);
        double g_lnT_ss = central_diff_logT((double)Taf, (double)Tas, (double)Tsf, (double)Tss, 3);
        bool ok = std::isfinite(g_lnT_af) && std::isfinite(g_lnT_as) && std::isfinite(g_lnT_sf) && std::isfinite(g_lnT_ss);
        if (!ok) {
          // Fallback to analytic contraction if any NaN/Inf
          fallback_analytic_timeconst();
        } else {
          gT_af.data_ptr<float>()[b] = (float)(g_lnT_af / Taf);
          gT_as.data_ptr<float>()[b] = (float)(g_lnT_as / Tas);
          gT_sf.data_ptr<float>()[b] = (float)(g_lnT_sf / Tsf);
          gT_ss.data_ptr<float>()[b] = (float)(g_lnT_ss / Tss);
        }
      } else {
        // Fixed-mask FD: reproduce legacy behavior for test compatibility
        auto scalar_loss_fixed = [&](float Raf_v, float Ras_v, float Rsf_v, float Rss_v) -> float {
          DiscreteAB ABa = zoh_discretize_from_rates(Raf_v, Ras_v, Rsf_v, Rss_v, Ts);
          DiscreteAB ABr = zoh_discretize_from_rates(0.f, 0.f, Rsf_v, Rss_v, Ts);
          float x1f=0.f, x2f=0.f, Lf=0.f;
        ...
        auto fd_rate = [&](float Raf_c, float Ras_c, float Rsf_c, float Rss_c, int which)->float{
          ...
          float minus[4] = {Raf_c, Ras_c, Rsf_c, Rss_c};
          const float q = base[which];
          // Use a larger epsilon for shunt_fast (which==2) and shunt_slow (which==3) to avoid float32 cancellation
          float scale = (which == 2 || which == 3) ? 1e-4f : 1e-6f;
          float epsr = scale * ((std::fabs(q) > 1.f) ? std::fabs(q) : 1.f);
          plus[which] = base[which] + epsr;
          minus[which] = base[which] - epsr;
